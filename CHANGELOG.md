...
(d) **Session and OAuth2 refresh now happens AFTER your `afterResponse`**. Before v10, the refresh happens before your `afterResponse`. This is a breaking change if your `afterResponse` captures 401 response status. See [Stale Authentication Credentials](https://github.com/zapier/zapier-platform/blob/master/packages/cli/README.md#stale-authentication-credentials) for details
