...
describe('create-app', () => {
  ...
  const createRawTestInput = (event) => createInput(appDefinition, event, testLogger);

  it('should return data from promise', (done) => {
    const input = createTestInput('resources.workingfuncpromise.list.operation.perform');
  ...
  it('should return data from promise (direct)', (done) => {
    const input = createTestInput('triggers.workingfuncpromiseList.operation.perform');
  ...
  it('should return data from an async function call with callback', (done) => {
    const input = createTestInput('resources.workingfuncasync.list.operation.perform');
  ...
  it('should return data from an async function call with callback (direct)', (done) => {
    const input = createTestInput('triggers.workingfuncasyncList.operation.perform');
  ...
  it('should return data from live request call, applying HTTP before middleware', (done) => {
    ...
    app(input).then(output => {
      ...
      output.results.headers['X-Hashy'].should.eql('1a3ba5251cb33ee7ade01af6a7b960b8');
  ...
  it('should make call via z.request', (done) => {
    ...
    app(input).then(output => {
      output.results[0].headers['X-Hashy'].should.eql('1a3ba5251cb33ee7ade01af6a7b960b8');
  ...
  it('should make call via z.request with sugar url param', (done) => {
    const input = createTestInput('triggers.requestsugarList.operation.perform');
  ...
  it('should fail on a sync function', (done) => {
    const input = createTestInput('resources.failerfunc.list.operation.perform');
  ...
  it('should fail on promise function', (done) => {
    const input = createTestInput('resources.failerfuncpromise.list.operation.perform');
  ...
  it('should fail on promise function (direct)', (done) => {
    const input = createTestInput('triggers.failerfuncpromiseList.operation.perform');
  ...
  it('should apply HTTP after middleware', (done) => {
    const input = createTestInput('resources.contacterror.listWithError.operation.perform');
  ...
  it('should return data from live request call (direct)', (done) => {
    ...
    app(input).then(output => {
      output.results.url.should.eql('http://zapier-httpbin.herokuapp.com/get');
      output.results.headers['X-Hashy'].should.eql('1a3ba5251cb33ee7ade01af6a7b960b8');
      output.results.headers['X-Author'].should.eql('One Cool Dev');
      done();
    }).catch(done);
  ...
  it('should return a rendered URL for OAuth2 authorizeURL', (done) => {
    ...
    oauth2App(input).then(output => {
      output.results.should.eql('http://my-sub.example.com?scope=read%2Cwrite');
      done();
    }).catch(done);
  });
  ...
  it('should run a raw provided request', (done) => {
    ...
    app(input).then(output => {
      ...
      JSON.parse(response.content).url.should.eql('http://zapier-httpbin.herokuapp.com/get');
      done();
    }).catch(done);
  });
  ...
  describe('HTTP after middleware for auth refresh', () => {
    it('should be applied to OAuth2 refresh app on shorthand requests', (done) => {
      ...
      oauth2App(createInput(oauth2AppDefinition, event, testLogger)).then(() => {
        done('expected an error, got success');
      }).catch(error => {
