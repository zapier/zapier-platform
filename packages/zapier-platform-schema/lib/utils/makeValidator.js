...
const makeLinks = (error, makerFunc) => {
  ...
  if (['anyOf', 'oneOf', 'allOf'].indexOf(error.name) !== -1 && error.argument && error.argument.length) {
...
const makeValidator = (mainSchema, subSchemas) => {
  ...
  return {
    validate: (definition) => {
      ...
      results.errors = results.errors.concat(functionalConstraints.run(definition));
