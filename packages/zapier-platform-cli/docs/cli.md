...
* `path [value]` -- **required**,
* `--template={minimal,resource,trigger,create,search,middleware,basic-auth,custom-auth,oauth2,session-auth,babel,rest-hooks,files,github,search-or-create}` -- _optional_, select a starting app template. Default is `minimal`
